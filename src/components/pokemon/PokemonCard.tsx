...
export interface PokemonCardData {
  id: number;           // dex number shown on card
...
export function PokemonCard({
  ...
  const { id, slug, displayName, spriteUrl, types, isOwned } = pokemon;
  ...
  return (
    <div className="group relative">
      {/* Main ownership toggle */}
      <button
        onClick={handleClick}
        className={cn(
          "relative flex w-full flex-col items-center gap-1.5 rounded-xl border p-3 text-left transition-all duration-200",
          ...
          collectionMode && !effectiveOwned && "hover:border-dex-owned/40",
      ...
      {/* Detail link — sibling to button so interactive elements don't nest */}
      {!collectionMode && (
        <Link
          href={`/pokemon/${slug}`}
          className={cn(
